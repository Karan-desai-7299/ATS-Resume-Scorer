...
def validate_file(file_data: bytes, filename: str) -> Tuple[bool, str, Optional[str]]:
    ...
    if file_size_bytes == 0:
        return (
            False,
            "Uploaded file is empty. Please upload a valid PDF or DOCX file.",
            None,
        )

    # Pure-Python byte signature detection (avoids native libmagic C dependency)
    file_type = None
    if file_data.startswith(b"%PDF-"):
        file_type = "pdf"
    elif file_data.startswith(b"PK\x03\x04"):
        # Zip container format (DOCX)
        file_type = "docx"
    elif file_data.startswith(b"\xd0\xcf\x11\xe0\xa1\xb1\x1a\xe1"):
        # OLE compound binary format (legacy DOC)
        file_type = "doc"
    else:
