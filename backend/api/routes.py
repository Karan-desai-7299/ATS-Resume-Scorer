...
async def generate_pdf(
    ...
    try:
        ...
        html_docs = generate_html_reports(raw_data)
        pdf_bytes = generate_combined_pdf(html_docs, data=raw_data)
...
async def generate_history_pdf(
    ...
    analysis_data = next((item["analysis_result"] for item in history if str(item.get("id")) == str(analysis_id)), None)

    if not analysis_data:
        logger.warning(f"Analysis ID {analysis_id} not found in user history for user {user_id}")
...
def _apply_rate_limit(user_id: str, max_requests: int = 10, window_seconds: int = 60):
    """Simple in-memory per-user rate limiter to protect Groq LLM API quota."""
    now = time.time()
    user_times = [t for t in _rate_limit_store[user_id] if now - t < window_seconds]
    _rate_limit_store[user_id] = user_times

    if len(user_times) >= max_requests:
        logger.warning(f"Rate limit exceeded for user {user_id}")
        raise HTTPException(
            status_code=429,
            detail=f"Rate limit exceeded. Maximum {max_requests} AI requests per minute allowed. Please wait a moment before trying again."
        )

    _rate_limit_store[user_id].append(now)

@router.post('/optimize-bullet')
async def optimize_bullet_endpoint(
    payload: dict,
    user_id: str = Depends(get_current_user),
):
    """Uses Groq LLaMA 3.3 70B to rewrite any draft bullet point into 3 high-impact ATS variations."""
    _apply_rate_limit(user_id, max_requests=10, window_seconds=60)

    from backend.services.groq_parser import optimize_bullet_point

    draft_bullet = payload.get('bullet', '').strip()
    target_role = payload.get('target_role', '').strip()

    if not draft_bullet:
        raise HTTPException(status_code=400, detail="Missing required field 'bullet'")

    try:
        result = optimize_bullet_point(draft_bullet, target_role)
        return result
    except Exception as exc:
        logger.error(f"Bullet optimization failed: {exc}")
        raise HTTPException(status_code=500, detail=f"Bullet optimization failed: {exc}")


@router.post('/ask-resume-ai')
async def ask_resume_ai(
    payload: dict,
    user_id: str = Depends(get_current_user),
):
    """Uses Groq LLaMA 3.3 as an AI career coach to answer questions about the user's resume analysis."""
    _apply_rate_limit(user_id, max_requests=10, window_seconds=60)

    from backend.services.groq_parser import ask_resume_question
    ...
    try:
        answer = ask_resume_question(question, analysis_context)
        return {"answer": answer}
    except Exception as exc:
        logger.error(f"ask_resume_ai failed: {exc}")
